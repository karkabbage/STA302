...
```{r}
coefficients <- coef(model_after_vif_no_outliers)
coefficients

#Assign coefficients from matrix to betas. Prepare for backtransformation
B0 <- coefficients[1]
tCRIM <- coefficients[2]
tINDUS <- coefficients[3]
tNOX <- coefficients[4]
tDIS <- coefficients[5]
tPTRATIO <- coefficients[6]
tB <- coefficients[7]
tLSTAT <- coefficients[8]
...
bt_coef <- c(B0, btCRIM, btINDUS, btNOX, btDIS, btPTRATIO, btB, btLSTAT)
